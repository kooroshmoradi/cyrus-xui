...
confirm_restart() {
    confirm "to restart the panel，restarting the panel will also restart xray" "y"
    if [[ $? == 0 ]]; then
        restart
    else
        show_menu
...
reset_user() {
    confirm "Are you sure you want to reset your username and password to admin NS" "n"
    if [[ $? != 0 ]]; then
        if [[ $# == 0 ]]; then
            show_menu
        fi
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            echo -e "${green}x-ui Stop with xray successfully${plain}"
        else
...
restart() {
    ...
    check_status
    if [[ $? == 0 ]]; then
        echo -e "${green}x-ui Restart successfully with xray${plain}"
    else
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        echo -e "${green}x-ui Set the power-on auto-start successfully${plain}"
    else
        echo -e "${red}x-ui Failed to set self-start after power-on${plain}"
    fi
...
disable() {
    systemctl disable x-ui
    if [[ $? == 0 ]]; then
        echo -e "${green}x-ui Cancel the power-on auto-start successfully${plain}"
    else
        echo -e "${red}x-ui Cancel startup failure${plain}"
    fi
...
update_shell() {
    ...
    if [[ $? != 0 ]]; then
        echo ""
        echo -e "${red}Failed to download the script, please check if the machine can be connected Github${plain}"
        before_show_menu
    else
...
show_enable_status() {
    check_enabled
    if [[ $? == 0 ]]; then
        echo -e "start automatically after booting: ${green}Yes${plain}"
    else
        echo -e "start automatically after booting: ${red}No${plain}"
    fi
}
...
show_usage() {
    ...
    echo "x-ui enable       - make x-ui enable"
    echo "x-ui disable      - make x-ui disable"
    echo "x-ui log          - View x-ui log"
    echo "x-ui v2-ui        - transfer v2-ui data to x-ui"
